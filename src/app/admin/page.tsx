  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="rounded-[28px] border border-white/[0.08] bg-surface p-6 sm:p-8 space-y-4">
        ...
        <p className="text-sm text-mist">Token-gated list — backend <span className="font-mono">BETA_ADMIN_TOKEN</span> se match hona chahiye. Local demo default: <span className="font-mono text-amber-300">previse-demo-admin</span> (prod me env var set karo).</p>
        ...
        {error && <p className="text-xs text-red-300">{error} — token galat hai ya backend :3001 down hai.</p>}
      ...
      {rows && (
        <div className="rounded-[24px] bg-surface border border-white/[0.08] divide-y divide-white/[0.06]">
          ...
          {rows.length === 0 && <p className="text-sm text-dusk text-center py-8">Abhi koi signup nahi.</p>}
        </div>
      )}
    </div>
  );
