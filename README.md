...
3. **Connect** (`/connect`) — AA 3-step flow (demo bank via `AA_PROVIDER=mock`, real Setu/OneMoney when wired) or CSV upload (`date, narration, amount, type`) → live profile auto-adopted by Dashboard + Simulator (● Live persona) + Delete My Data (DPDP). No statement handy? Select a sample (Spender/Saver) to try instantly — same upload + parser flow, previous data replaced.
