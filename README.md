...
1. **Dashboard** (`/`) — Emergency Buffer · Runway · Financial Firewall bar · Goals · profile pill (active manual/live profile + create/delete)
...
3. **Connect** (`/connect`) — AA 3-step flow (demo bank via `AA_PROVIDER=mock`, real Setu/OneMoney when wired) or CSV upload (`date, narration, amount, type`) → live profile auto-adopted by Dashboard + Simulator (● Live persona) + Delete My Data (DPDP). No statement handy? Create a manual profile from the navbar to try instantly. Production mode: no demo personas, no mock fallback — 404 until you connect.
4. **Goals** (`/goals`) — add (blank-slate form, no pre-filled values) / edit / delete goals + per-goal delay impact
...
backend/
  ...
  data/                # test-profile.js (QA/engine fixture ONLY, never served) + sessions/signups (gitignored, runtime)
...
src/
  ...
  store/               # useFinanceStore (manual profiles + live data + goals CRUD + history, persisted; v3 self-heal + 1h liveData TTL, no DB)
  lib/api.ts           # backend client + adapter (single source of truth via /api/simulate/custom)
  lib/engine.ts        # frontend engine (backend parity) + engine.test.ts (8/8) + insights.test.ts (6/6) + store/persist.test.ts (7/7) = vitest 21/21
...
| GET | `/api/profile/live` | Profile + financial state + parsed transactions + accounts (404 `NO_LIVE_DATA` until AA/CSV connected — no demo fallback) |
...
| `npx vitest run` | **21/21** — frontend↔backend engine parity (EMI math, verdict rules, loan branch, firewall) + insights + persist self-heal (stale-state reset, 1h liveData TTL) |
